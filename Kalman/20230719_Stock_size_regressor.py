...
path_str = 'C:\Ráðgjöf\Maris Optimum\gr_data\\'
...
#    X_cal_df = X_cal_df.mul(catch_df.number*-1e6, axis=0)
    ...
    YZ = pd.read_csv("C:\\Ráðgjöf\\Maris Optimum\\gr_data\\stock_index.csv", sep=";")
    YY = YZ.iloc[:,0]
...
def stock_weight_converter(X_catch_per_df, catch_df): 
    ...
    """
    path_grs_str = 'C:\Ráðgjöf\Maris Optimum\Golden_redfish_model/'
    path_grs_weight =\
        'C:\Ráðgjöf\Maris Optimum\Data_From_Hafro_20220906\ssb_weights/'
...
fractile = '101'#current_prediction = 0
L = []
M = []
(X_Observed, y_Observed) = get_new_data(fractile)
